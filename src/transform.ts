...
const ALLOWED_KEYS = ['name', 'platform', 'description', 'resolution', 'query'] as const;
 ...
 */
export function filterByLevel(items: PolicyItem[], level: string): PolicyItem[] {
  if (level !== '1' && level !== '2') {
 ...
 */
export function toYamlChunks(items: SanitizedPolicyItem[]): Record<string, string> {
  const options: yaml.ToStringOptions = {
    indent: 2,
    lineWidth: 0, // Prevent line wrapping
    minContentWidth: 0,
