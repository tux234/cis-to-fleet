...
test('outputPath generates correct file paths', () => {
  expect(outputPath('macos-15', './output')).toBe(join('./output', 'cis-benchmark-macos15.yml'));
  expect(outputPath('windows-11', '/tmp')).toBe(join('/tmp', 'cis-benchmark-windows11.yml'));
...
test('write throws error when file exists and overwrite is false', async () => {
  ...
  try {
    ...
    expect(write('new content', testFile, false)).rejects.toThrow('File already exists');
  } finally {
    // Cleanup
    try {
      await unlink(testFile);
